...
public class MyMallMain {
	...
	public static void main(String[] args) {
		System.out.println("::::::::::::::: 김땡땡 쇼핑몰에 오신 것을 환영합니다 :::::::::::::::");
		...
		System.out.println("\n상품 구매를 위한 로그인(필수) >>");
		while(!isLogin) {
	        System.out.println("간편로그인 - 사용자 ID 입력하세요(로그인 취소는 0000)>>   ");
...
//		--4. 상품 장바구니 담기 - 장바구니 테이블이 없으므로 구매를 원하는 상품을 List 에 담기
		...
		if(isLogin) {
			...
			while(true) {
				...
				System.out.println("장바구니에 담긴 상품을 결제하시겠습니까?");
				if(sc.nextLine().toLowerCase().equals("y/Y")) break;
				}
			System.out.println("장바구니 확인 : "+carts);
			for(JBuy b : carts) {
				System.out.println("상품코드 :"+b.getPcode()+"수량 : "+b.getQuantity());
			}
			//dao에서 carts를 전달받아 list 만큼 반복하는 insert 실행하기
			int count = bdao.insertMany(carts);
			if(count!=0)
				
			System.out.println("\n"+" 결제를 완료했습니다. ");
			//5번에 장바구니 담긴 상품이 j_buy 테이블에 1) 정상저장, 2) 잘못된 수량 rollback까지 되는지 확인
			//6. 마이페이지 - 구매내역보기. 총 구매 금액을 출력해주기 -> sql 테스트 해보고 메소드 작성 시도하기
			System.out.println(":::: 현재까지 "+customer.getName()+"회원님으 ㅣ구매 내역 입니다. ::::");
			try {
